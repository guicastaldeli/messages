...
export class ChunkRenderer {
    ...
    */
    public async setupScrollHandler(): Promise<void> {
        ...
        if(this.messageManager.scrollHandler) {
            container.removeEventListener('scroll', this.messageManager.scrollHandler);
        }
    ...
    */
    public async loadChunk(): Promise<void> {
        ...
        const chatId = this.messageManager.currentChatId;
        const cacheService = this.messageManager.getCacheService();
        const cacheData = cacheService.getCacheData(chatId);
        let nextPage = this.messageManager.currentPage + 1;

        if (!cacheData || !cacheService.hasMoreMessages(chatId)) return;
        this.currentLoadingPage = nextPage;
        await this.messageManager.loadHistory(chatId, nextPage);
        this.currentLoadingPage = -1;
    }
    ...
    public async loadCachedPages(chatId: string): Promise<void> {
        ...
        for(const page of loadedPages) {
            ...
            if(messages.length > 0) {
                await this.messageManager.getMessageElementRenderer()
