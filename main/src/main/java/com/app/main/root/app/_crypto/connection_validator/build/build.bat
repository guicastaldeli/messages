...
del *.obj 2>nul
del connection_validator.dll 2>nul

echo.
echo Compiling with CL.EXE...
cl /nologo /c /O2 /EHsc /std:c++17 /I"%JAVA_HOME%\include" /I"%JAVA_HOME%\include\win32" /I"%OPENSSL_INCLUDE%" ..\connection_validator.cpp
if %errorlevel% neq 0 (
    echo ERROR: Failed to compile connection_validator.cpp
    pause
    exit /b 1
)

cl /nologo /c /O2 /EHsc /std:c++17 /I"%JAVA_HOME%\include" /I"%JAVA_HOME%\include\win32" /I"%OPENSSL_INCLUDE%" ..\connection_validator_jni.cpp
if %errorlevel% neq 0 (
    echo ERROR: Failed to compile connection_validator.cpp
    pause
    exit /b 1
...
link /nologo /DLL /OUT:connection_validator.dll connection_validator.obj connection_validator_jni.obj /LIBPATH:"%OPENSSL_LIB%" libssl.lib libcrypto.lib ws2_32.lib gdi32.lib crypt32.lib advapi32.lib
...
if exist connection_validator.dll (
