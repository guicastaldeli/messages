...
std::string Base64Manager::base64Encode(const std::vector<unsigned char>& data) {
...
std::vector<unsigned char> Base64Manager::base64Decode(const std::string& encoded) {
    const std::string base64_chars = 
        "ABCDEFGHIJKLMNOPQRSTUVWXYZ"
        "abcdefghijklmnopqrstuvwxyz"
        "0123456789+/";

    std::string standard_encoded = encoded;
